...
if (!canEditUsers($_SESSION['user_id'])) {
  echo '<div class="p-6"><h1 class="text-2xl font-bold text-red-600">Access Denied</h1><p>You do not have permission to manage reservation purposes.</p></div>';
  return;
}
try {
  $purposes = $pdo->query("
...
?>
<div class="space-y-6 p-3 sm:p-0">
  <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center space-y-4 sm:space-y-0">
    <div>
      <h1 class="text-2xl sm:text-3xl font-bold text-gray-900">Reservation Purposes</h1>
      <p class="text-gray-600">Manage the different purposes for room reservations</p>
    </div>
    <?php if (canEditUsers($_SESSION['user_id'])): ?>
      <button onclick="loadPage('CreatePurpose')"
        class="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg flex items-center justify-center sm:justify-start touch-manipulation">
      ...
      <?php foreach ($purposes as $purpose): ?>
        <div class="bg-white rounded-lg shadow p-4 border">
          <div class="flex justify-between items-start mb-3">
            <div class="flex-1">
              <h3 class="text-lg font-semibold text-gray-900"><?php echo htmlspecialchars($purpose['name']); ?></h3>
              <p class="text-sm text-gray-600 mt-1"><?php echo htmlspecialchars($purpose['description'] ?? 'No description'); ?></p>
            </div>
            <div class="flex space-x-2 ml-4">
              ...
              })()" class="p-2 text-gray-400 hover:text-blue-600 touch-manipulation" title="Edit Purpose">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z">
                  </path>
              ...
              })()" class="p-2 text-gray-400 hover:text-red-600 touch-manipulation" title="Delete Purpose">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16">
                  </path>
          ...
          <div class="grid grid-cols-2 gap-4 text-sm">
            ...
            <div>
              ...
              <span class="text-gray-900 ml-2"><?php echo $purpose['usage_count']; ?> reservation<?php echo $purpose['usage_count'] !== 1 ? 's' : ''; ?></span>
            </div>
            <div class="col-span-2">
              <span class="text-gray-500">Created:</span>
              <span class="text-gray-900 ml-2"><?php echo date('M j, Y', strtotime($purpose['created_at'])); ?></span>
            </div>
          </div>
        </div>
      <?php endforeach; ?>
