...
<form id="edit-room-type-form" class="space-y-6 max-w-2xl mx-auto p-6 bg-white rounded-md shadow-md">
  ...
  <div class="bg-blue-50 border border-blue-200 rounded-md p-4">
    <div class="flex items-center">
      ...
      <span class="text-sm text-blue-800">
        This room type is currently used by <strong><?php echo $roomCount; ?></strong>
        room<?php echo $roomCount !== 1 ? 's' : ''; ?>
      </span>
    </div>
  </div>
  <div>
  ...
  <div>
    ...
    <div class="flex items-center space-x-3">
      <input id="color" name="color" type="color" required value="<?php echo htmlspecialchars($roomType['color']); ?>"
  ...
  <div>
    ...
    <div class="grid grid-cols-6 gap-2">
      ...
      <button type="button" onclick="document.getElementById('color').value = '<?php echo $colorCode; ?>'"
        ...
        style="background-color: <?php echo $colorCode; ?>" title="<?php echo $colorName; ?>">
      </button>
      <?php endforeach; ?>
    </div>
  </div>
    ...
    <div class="flex">
      ...
      <div class="ml-3">
        ...
        <div class="mt-2 text-sm text-yellow-700">
          <p>Changes to this room type will affect all <?php echo $roomCount; ?>
            room<?php echo $roomCount !== 1 ? 's' : ''; ?> that use this type. The color and name changes will be
            reflected immediately across the system.</p>
        </div>
      </div>
    </div>
  </div>
...
document.getElementById('description').addEventListener('input', function() {
  const value = this.value.trim();
  if (value.length < 10) this.classList.add('border-red-300');
  else this.classList.remove('border-red-300');
});
</script>