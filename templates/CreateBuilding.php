    ...
    <button type="button" onclick="(async function() {
        ...
        try {
          ...
          if (response.ok && result.building_id) {
            ...
            if (result.errors) popupSystem.error(result.errors.join('\\n'));
      ...
      class="px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500">Create
      Building</button>
  </div>
</form>
...
<script>
  ...
  document.getElementById('description').addEventListener('input', function() {
    const value = this.value.trim();
    if (value.length > 1000) this.classList.add('border-red-300');
    else this.classList.remove('border-red-300');
  });
  document.getElementById('address').addEventListener('input', function() {
    const value = this.value.trim();
    if (value.length > 255) this.classList.add('border-red-300');
    else this.classList.remove('border-red-300');
  });
</script>